...
| **P5 ✅** | `snapshot.rs`: snapshot metadata + name trees, point-in-time volume view — DONE (snap-meta tree located by the APSB physical block# @152, walked with checksum/cycle guards; `j_snap_metadata_val_t` offsets verified vs libfsapfs `fsapfs_snapshot_metadata_btree_value` **and** dissect.apfs — a RED test caught an early name@48→@50 off-by-2; validated on the real P4 fixture: empty snap-meta tree at block 340 yields zero snapshots, `mount_snapshot` of block 438 == direct `ApfsVolume::parse` (point-in-time seam); populated `listSnapshots` + v1-vs-v2 point-in-time read are env-gated on `APFS_P5_FIXTURE` because snapshot *creation* on a DMG is SIP-blocked — see §6 note + `docs/validation.md`) | `diskutil apfs listSnapshots`; fsapfsinfo |
| **P6 ✅** | `spaceman.rs` + `reaper.rs`: allocation bitmaps (`is_block_free`), reaper pending-objects — DONE (bit polarity 1=allocated cross-checked vs apfsprogs/linux-apfs; validated on the real container — blocks 0..=344 allocated, ≥345 free, and the all-blocks allocation count == `block_count − free_count` = 345, an oracle independent of the bitmap read; reaper empty on the clean fixture + synthetic reap-list walk; ephemeral `nx_spaceman_oid`/`nx_reaper_oid` resolved via the checkpoint map. CAB multi-TB tier fails loud (`UnsupportedSpacemanCab`). See `docs/validation.md` P6) | `apfsck` spaceman; spaceman free-count |
| **P7 ✅** | `encryption.rs`: keybag state surfacing (tags, passphrase hint, raw unknown tags) — DONE; no key derivation. Synthetic spec-faithful keybags (no committed encrypted volume) | apfs-fuse keybag; macOS `diskutil apfs` |
| **P8 ✅** | `sealed.rs` (`IntegrityMeta::parse` incl. `im_broken_xid`) — DONE; `fusion.rs::translate_address` fails loud (full Fusion needs a fixture, rejected at open). File-info hashes deferred to a real SSV | real SSV image; `apfsck` |
| **P9 ✅** | `apfs-forensic` analyzer — DONE: `Observation` grading per the table; audits for integrity (XID-REUSE), snapshots, recovery (reaper), crypto, sealed (broken seal), clones; `audit_container`/`audit_volume` drivers (clean fixture → no High anomalies). SSV-hash / extentref-shared-block / broader recovery leads scoped to their validating corpora | end-to-end on minted + real corpora |
